...
def iterate(func, current_output, my_list):
    
    for item in my_list:
          current_output = func(current_output, item)
...
def parens_match_scan(mylist):
    ...
    """
    ###TODO
    history, last = scan(plus, list(map(paren_map, mylist)), 0)
...
def scan(f, id_, a):
    ...
    """
    return (
            [reduce(f, id_[:i+1], a) for i in range(len(id_))],
             reduce(f, id_, a)
           )
